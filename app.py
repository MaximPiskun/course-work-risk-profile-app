...
def _build_personal_feedback(
        ...
        cards.append(
            {
                ...
                "fact": f"Индекс хаотичности — {chaos:.2f}; направление решений часто менялось.",
                "meaning": "Из-за этого оценка профиля становится менее устойчивой.",
                "advice": "Задайте базовую стратегию заранее и меняйте аллокацию только при существенных изменениях рынка.",
            }
        )
        ...
        cards.append(
            {
                ...
                "fact": f"За эпизод было {panic} панических действий после рыночных шоков.",
                "meaning": "Потери заметно влияли на решения: после просадки вы резко снижали риск.",
                "advice": "Заранее определите допустимую просадку и план действий, чтобы не принимать импульсивные решения.",
            }
        )
        ...
        cards.append(
            {
                ...
                "fact": f"Показатель восстановления риска: {restore:.0f} из 1.",
                "meaning": "После стрессовых тиков вы надолго оставались в защитной позиции.",
                "advice": "Возвращайте риск к целевому уровню по шагам после стабилизации рынка.",
            }
        )
        ...
        cards.append(
            {
                ...
                "fact": f"Средняя доля рискованных активов: {avg_risky:.0%}; панических действий не было.",
                "meaning": "Вы стабильно переносили рыночные колебания.",
                "advice": "Сохраняйте дисциплину: проводите ребаланс по правилам, а не по эмоциям.",
            }
        )
    ...
    if mismatch:
        cards.append(
            {
                "title": "Анкета и поведение",
                "fact": "Профиль из анкеты не совпал с действиями в симуляции.",
                "meaning": "Самооценка риска может отличаться от реального поведения в динамике.",
                "advice": "При формировании плана лучше опираться на фактическое поведение в симуляции.",
            }
        )
        ...
        cards.append(
            {
                ...
                "fact": "Выраженных стрессовых реакций не выявлено.",
                "meaning": "Для этого эпизода поведение выглядит достаточно ровным.",
                "advice": "Повторите тест в другом режиме и сравните результат.",
            }
        )
...
def render_results() -> None:
    ...
    st.markdown("### Персональные выводы")
    ...
    for idx, card in enumerate(feedback_cards, start=1):
        ...
        st.markdown(f"- **Наблюдение:** {card['fact']}")
        st.markdown(f"- **Интерпретация:** {card['meaning']}")
        st.markdown(f"- **Рекомендация:** {card['advice']}")
